...
export const image = {
  convertToWebp: () => {
    return app
      .src(`${app.path.temp.filesToConvert}.{jpg,jpeg,png,webp,gif}`)
      .pipe(
        app.plugins.plumber(
  ...
  copySourceImage: () => {
    return app
      ...
      .pipe(app.plugins.if(app.isBuild, app.dest(app.path.temp.img)))
  ...
  copySvgImage: () => {
    return app
      ...
      .pipe(app.plugins.if(app.isBuild, app.dest(`${app.path.temp.img}/svg`)))
      .pipe(app.plugins.bs.stream());
  },

  optimizeImage: () => {
    return app
      .src(`${app.path.temp.img}/**/*`)
      .pipe(
        app.plugins.plumber(
          app.plugins.notify.onError({
            title: "IMAGES:COPY SVG IMAGES",
