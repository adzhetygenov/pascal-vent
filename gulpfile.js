...
function watcher() {
  ...
  gulp.watch(path.watch.view, view).on("all", (event, changed) => {
    ...
    app.plugins.log(
      `${app.plugins.chalk.bold.green(changed)} is being compiled`
    );
...
const images = app.isBuild
  ...
  : gulp.series(image.copySourceImage, image.copySvgImage);

const main = gulp.series(
  watchInit,
  font,
  gulp.parallel(style.custom, style.vendor, js, images),
  view
);

const build = gulp.series(clean, copy, data, main);

const dev = gulp.series(clean, copy, data, main, server, watcher);
